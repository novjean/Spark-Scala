...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    println("MOST POPULAR SUPERHEROES: ")
    val mostPopular10 = flipped.takeOrdered(10)(Ordering[(Int,Int)].reverse)
    for(result <- mostPopular10)
      {
        val name = namesRdd.lookup(result._2)(0)
        val count = result._1
    ...
    println("LEAST POPULAR SUPERHEROES: ")
    val leastPopular10 = flipped.takeOrdered(10)(Ordering[(Int,Int)])
    for(result <- leastPopular10)
    {
      val name = namesRdd.lookup(result._2)(0)
      val count = result._1
      println(s"$name : $count")
    ...
    */
     /* val mostPopular = flipped.sortByKey()
        
      val results = mostPopular.collect()
